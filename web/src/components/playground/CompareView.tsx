...
import type { ChatCompletionMessage, GatewayModel } from "@/types/api";
          ...
          {columns.length < MAX_COLUMNS && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-primary hover:bg-primary/10 rounded-md transition-colors cursor-pointer"
              >
                <Plus size={14} />
                添加模型
              </button>
              {showAddModel && (
                <div className="absolute right-0 top-full mt-1 w-60 bg-card border border-border rounded-lg shadow-elevated z-50 max-h-64 overflow-y-auto py-1">
                  {models.map((m) => (
                    <button
                      key={m.name}
                      onClick={() => addColumn(m)}

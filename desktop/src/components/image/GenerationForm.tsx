...
import type { SubmitTaskRequest, EditTaskRequest, ImageTaskParams } from '@/lib/types-api';
import type { GatewayModel } from '@/lib/types-api';
...
type ParamField = { key: string; label: string; options: SelectOption[]; default: string };
type ModelSpec = { fields: ParamField[]; fixedSize?: string; sizeFieldKey?: string };
...
function getModelSpec(model: string): ModelSpec {
  ...
  if (isGeminiImage) {
    const ratios = isFlash ? [...GEMINI_BASE_RATIOS, ...GEMINI_FLASH_EXTRA_RATIOS] : GEMINI_BASE_RATIOS;
    const resolutions: SelectOption[] = isFlash
      ? [{ value: '512', label: '512' }, { value: '1K', label: '1K' }, { value: '2K', label: '2K' }, { value: '4K', label: '4K' }]
      : [{ value: '1K', label: '1K' }, { value: '2K', label: '2K' }, { value: '4K', label: '4K' }];
    return {
      fields: [
        { key: 'aspect_ratio', label: '宽高比', options: ratios, default: '1:1' },
        { key: 'image_resolution', label: '分辨率', options: resolutions, default: isProImage ? '2K' : '1K' },
      ],
      fixedSize: '1024x1024',
    };
  }
  ...
  if (isGptImage) {
    return {
      fields: [
        { key: 'size', label: '尺寸', options: GPT_IMAGE_SIZE_OPTIONS, default: '1280x1280' },
        { key: 'quality', label: '质量', options: [
...
// Native styled select for obsidian dark theme
function NativeSelect({ id, value, onChange, options }: {
  ...
  return (
    <select
      ...
      className="w-full h-8 px-2 text-xs bg-obsidian-800 border border-obsidian-700 rounded-md text-obsidian-100 focus:outline-none focus:border-amber-500"
    >
      {options.map((o) => (
...
export function GenerationForm({ availableModels, editImageUrl, onEditImageConsumed }: GenerationFormProps) {
  ...
  // Load editImageUrl as a reference image (fetch directly — desktop webview supports https:// TOS URLs)
  useEffect(() => {
    if (!editImageUrl) return;
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch(editImageUrl);
        if (cancelled) return;
  ...
  const removeImage = (index: number) => {
    ...
    if (referenceImages.length <= 1) setMaskImage(null);
  };
  ...
  const buildSubmitPayload = (): { size: string; params?: ImageTaskParams } => {
    ...
    for (const f of spec.fields) {
      ...
      if (sizeFieldKey === f.key) {
        if (/^\d+x\d+$/.test(v)) size = v;
        else { size = '1024x1024'; params[f.key] = v; }
      } else {
        params[f.key] = v;
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (prompt.length > 2000) { toast.error('提示词长度不能超过 2000 个字符'); return; }
    ...
    try {
      if (isEditing) {
        const request: EditTaskRequest = { model, prompt: prompt.trim(), size, n: count, images: referenceImages, mask: maskImage || undefined, params };
        await submitEditTask.mutateAsync(request);
      } else {
        const request: SubmitTaskRequest = { model, prompt: prompt.trim(), size, n: count, params };
  ...
  if (!availableModels || availableModels.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <Image size={32} className="text-obsidian-500 mb-3" />
        <p className="text-sm text-obsidian-400">暂无可用的图片生成模型</p>
        <p className="text-xs text-obsidian-500 mt-1">请联系管理员配置</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="text-center pb-2">
        <div className="w-10 h-10 bg-pink-600 rounded-xl flex items-center justify-center mx-auto mb-2.5">
          <Image size={20} className="text-white" />
        </div>
        <h2 className="text-base font-semibold text-obsidian-50">{isEditing ? '图片编辑' : '图片生成'}</h2>
      ...
      <div className="space-y-1.5">
        <label htmlFor="prompt" className="text-xs font-medium text-obsidian-200">提示词</label>
        <textarea
          id="prompt"
          ...
          rows={4}
          maxLength={2000}
          className="w-full px-3 py-2 text-sm border border-obsidian-700 rounded-lg bg-obsidian-800 text-obsidian-100 resize-none focus:outline-none focus:border-amber-500 placeholder:text-obsidian-500"
        />
        <div className="text-[11px] text-obsidian-500 text-right">{prompt.length} / 2000</div>
      </div>

      {/* Image Upload */}
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-obsidian-200">
          参考图片 <span className="text-obsidian-500 font-normal">（可选，上传后进入编辑模式）</span>
        </label>
        <div
          className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all duration-200 ${
            dragOver ? 'border-pink-500 bg-pink-500/5' : 'border-obsidian-700 hover:border-obsidian-500 hover:bg-obsidian-800/50'
          }`}
        ...
        >
          ...
          />
          <Upload className="w-5 h-5 mx-auto mb-1.5 text-obsidian-500" />
          <p className="text-xs text-obsidian-400">点击或拖拽上传 · PNG/JPEG/WebP · 最多 {MAX_IMAGES} 张 · ≤ 25MB</p>
        </div>
            ...
            {referenceImages.map((file, index) => (
              <div key={index} className="relative group w-16 h-16">
                <img
                  ...
                  alt={`参考图 ${index + 1}`}
                  className="w-full h-full object-cover rounded-lg border border-obsidian-700"
                />
                <button
                  type="button"
          ...
          <div className="mt-2 space-y-1">
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1 text-xs border border-obsidian-700 rounded-md text-obsidian-200 hover:bg-obsidian-800 transition-colors cursor-pointer"
              >
            ...
            <input
              ...
              className="hidden"
              accept="image/png"
              onChange={(e) => { const f = e.target.files?.[0]; if (f) setMaskImage(f); e.target.value = ''; }}
            />
            <p className="text-[11px] text-obsidian-500">PNG 格式，透明区域表示需要编辑的位置</p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="space-y-1.5">
          <label htmlFor="model" className="text-xs font-medium text-obsidian-200">模型</label>
          <NativeSelect
            id="model"
            value={model}
            onChange={setModel}
            options={availableModels.map((m) => ({ value: m.name, label: m.display_name || m.name }))}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {spec.fields.map((field) => (
            <div key={field.key} className="space-y-1.5">
              <label htmlFor={field.key} className="text-xs font-medium text-obsidian-200">{field.label}</label>
              <NativeSelect
                id={field.key}
          ...
          <div className="space-y-1.5">
            <label htmlFor="count" className="text-xs font-medium text-obsidian-200">数量</label>
            <NativeSelect
              id="count"
              value={count.toString()}
              onChange={(v) => setCount(parseInt(v))}
              options={COUNTS.map((c) => ({ value: c.value.toString(), label: c.label }))}
            />
          </div>
        </div>
      </div>
      ...
      >
        {isPending ? (
          ...
          <><Sparkles className="w-4 h-4" />{isEditing ? '提交编辑任务' : '提交生成任务'}</>
        )}
      </button>

      <p className="text-[11px] text-center text-obsidian-500">Ctrl + Enter 快速提交</p>
    </form>
  );
}

...
const ModelColumn = ({
  ...
  return (
    <div className="flex-1 min-w-0 border-r border-obsidian-700 last:border-r-0 flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-obsidian-700 bg-obsidian-800/30">
        <div>
          <div className="text-sm font-medium text-obsidian-100 truncate">{displayName || model}</div>
          <div className="text-xs text-obsidian-400">temp: {temperature}</div>
        </div>
        <div className="flex items-center gap-1">
          {sse.streaming && (
            <button
              onClick={() => sse.abort()}
              className="p-1 rounded hover:bg-red-500/10 hover:text-red-400 text-obsidian-400 transition-colors cursor-pointer"
              title="停止生成"
            >
              <Square size={14} />
            </button>
      ...
      <div ref={scrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-3">
        ...
        {sse.streaming && response && (
          <span className="inline-block w-1.5 h-4 bg-obsidian-100/60 animate-pulse ml-0.5" />
        )}
        {error && (
      ...
      {/* Footer metrics */}
      {usage && !sse.streaming && (
        <div className="px-3 py-2 border-t border-obsidian-700 text-xs text-obsidian-400 flex items-center gap-3">

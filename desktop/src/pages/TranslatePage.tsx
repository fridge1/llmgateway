...
import type { SSEUsage } from "@/lib/types-api";

const headerSelectClass =
  "h-7 px-2 text-xs border border-obsidian-700 rounded-md bg-obsidian-900 text-obsidian-100 focus:outline-none focus:ring-1 focus:ring-amber-500";

const langSelectClass =
  "h-8 px-3 text-sm border border-obsidian-700 rounded-lg bg-obsidian-900 text-obsidian-100 focus:outline-none focus:ring-1 focus:ring-amber-500";
...
const TranslatePage = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-obsidian-950">
      ...
      <header className="h-12 flex items-center justify-between px-4 border-b border-obsidian-700 bg-obsidian-900 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-semibold text-sm text-obsidian-50">文本翻译</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <select
      ...
      {/* Language bar */}
      <div className="flex items-center justify-center gap-4 px-4 py-2 border-b border-obsidian-700 bg-obsidian-900/50 shrink-0">
        ...
        <button
          ...
          className="p-1.5 rounded-lg text-obsidian-400 hover:text-obsidian-100 hover:bg-obsidian-800 transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
          title="交换语言"
        >
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Source panel */}
        <div className="flex-1 flex flex-col border-r border-obsidian-700">
          <textarea
            value={sourceText}
            onChange={(e) => setSourceText(e.target.value)}
            onKeyDown={handleKeyDown}
            placeholder="输入要翻译的文本..."
            className="flex-1 p-4 text-sm bg-obsidian-950 text-obsidian-100 placeholder:text-obsidian-500 resize-none focus:outline-none"
          />
          <div className="flex items-center justify-between px-4 py-2 border-t border-obsidian-700 bg-obsidian-900/50">
            <span className="text-xs text-obsidian-400">{sourceText.length} 字符</span>
            <button
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex-1 p-4 text-sm bg-obsidian-950 text-obsidian-100 overflow-y-auto whitespace-pre-wrap">
            {error ? (
              <span className="text-red-400">错误: {error}</span>
            ) : translatedText ? (
              translatedText
            ) : (
              <span className="text-obsidian-500">翻译结果将显示在这里...</span>
            )}
          </div>
          <div className="flex items-center justify-between px-4 py-2 border-t border-obsidian-700 bg-obsidian-900/50">
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 text-xs text-obsidian-400 hover:text-obsidian-100 rounded-md hover:bg-obsidian-800 transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
            >
